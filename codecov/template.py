...
class CommentLoader(jinja2.BaseLoader):
    ...
    def get_source(self, environment: jinja2.Environment, template: str) -> tuple[str, str | None, Callable[..., bool]]:
        if template == 'base':
            return (
                self.base_template,
...
def get_comment_markdown(
    *,
    ...
    env.filters['file_url'] = functools.partial(get_file_url, repo_name=repo_name, pr_number=pr_number)
    env.filters['file_base_url'] = functools.partial(get_file_base_url, repo_name=repo_name, base_ref=base_ref)
...
def get_file_base_url(
    ...
    *,
    ...
    if lines is not None:
        # L stands for Left side of the diff. But since we generate these links for base code we only need the left side.
