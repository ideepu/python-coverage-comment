...
from codecov.coverage.jest import JestCoverage, JestFileCoverage
from codecov.coverage.pytest import PytestCoverage, PytestFileCoverage
...
class FileInfo:
    path: pathlib.Path
    coverage: PytestFileCoverage | JestFileCoverage
...
def get_comment_markdown(  # pylint: disable=too-many-arguments,too-many-locals,too-many-positional-arguments
    ...
    **kwargs: Any,
    ...
    # TODO: Check this code and see the groups are correctly generated
...
def sort_order(file_info: FileInfo) -> tuple[int, int, int]:
    ...
    new_covered_lines = len(file_info.coverage.covered_lines)

    return abs(new_missing_lines), added_statements, abs(new_covered_lines)


def read_template_file(template: str) -> str:
    return (resources.files(__package__) / 'template_files' / template).read_text()
